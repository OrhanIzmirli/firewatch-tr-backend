...
import {
  ...
  halfSwathKm,
  distanceKm,
  NADIR_FRACTION_LIMIT,
} from '../services/swathGeometry';
...
const TLE_CACHE_DIR = path.join(process.cwd(), '.tle-cache');
...
async function loadTle(sensor: Sensor): Promise<string> {
  if (!fs.existsSync(TLE_CACHE_DIR)) fs.mkdirSync(TLE_CACHE_DIR, { recursive: true });
  const file = path.join(TLE_CACHE_DIR, `${sensor.tleName.replace(/[^\w]/g, '_')}.tle`);
  if (fs.existsSync(file)) {
    const ageHours = (Date.now() - fs.statSync(file).mtimeMs) / 3_600_000;
    if (ageHours < 12) return fs.readFileSync(file, 'utf8');
  }
  const response = await axios.get<string>(CELESTRAK, {
...
interface Detection {
  lat: number; lon: number; time: Date; satelliteCode: string; product: string;
}

function parseCsv(csv: string, product: string): Detection[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!Number.isFinite(lat) || !Number.isFinite(lon) || !date || !Number.isFinite(raw)) continue;
...
function turkeyWindows(track: TrackPoint[], sensor: Sensor): TrackPoint[][] {
  const BOX = { minLat: 35.8, maxLat: 42.2, minLng: 25.6, maxLng: 44.8 };
  const windows: TrackPoint[][] = [];
  let current: TrackPoint[] = [];
  let lastIdx = -2;

  track.forEach((p, i) => {
    const half = halfSwathKm(p.altKm, sensor.scanAngleDeg);
    ...
    if (distanceKm(p.latDeg, p.lonDeg, clampedLat, clampedLon) > half * 1.05) return;
    if (i !== lastIdx + 1 && current.length) { windows.push(current); current = []; }
    current.push(p);
    lastIdx = i;
  });
...
function fmt(d: Date): string { return d.toISOString().slice(11, 16); }
...
async function main(): Promise<void> {
  ...
  console.log(`step    ${STEP_SECONDS}s   nadir limit ${NADIR_FRACTION_LIMIT}\n`);

  // ---- 1. orbits -----------------------------------------------------------
  const tracks = new Map<string, TrackPoint[]>();
  const windowsBySensor = new Map<string, TrackPoint[][]>();
  let propagateMs = 0;

  for (const sensor of SENSORS) {
    let tle: string;
    try { tle = await loadTle(sensor); }
    catch (e) { console.log(`${sensor.name.padEnd(11)} TLE FETCH FAILED ${(e as Error).message}`); continue; }
    const satrec = parseTle(tle, sensor.tleName);
    if (!satrec) { console.log(`${sensor.name.padEnd(11)} TLE PARSE FAILED`); continue; }

    const t0 = Date.now();
    const track = groundTrack(satrec, start, end, STEP_SECONDS);
    ...
    console.log(
      `${sensor.name.padEnd(11)} passes over Turkey=${String(windows.length).padStart(2)}  ` +
        `half-swath=${halfSwathKm(track[0]?.altKm ?? 830, sensor.scanAngleDeg).toFixed(0)}km  ` +
  ...
  if (!apiKey) { console.log('\nNASA_API_KEY not set — skipping validation'); return; }
  ...
  for (const product of ['VIIRS_SNPP_NRT', 'VIIRS_NOAA20_NRT', 'VIIRS_NOAA21_NRT', 'MODIS_NRT']) {
    try {
      ...
      const { data } = await axios.get<string>(url, { timeout: 30_000, responseType: 'text' });
      const rows = parseCsv(data, product);
      detections.push(...rows);
      perProduct.set(product, rows.length);
    } catch (e) {
      console.log(`  ${product} fetch failed: ${(e as Error).message}`);
    }
  }
  ...
  for (const sensor of SENSORS) {
    ...
    for (const w of windows) {
      const from = w[0].time.getTime() - 300_000;
      const to = w[w.length - 1].time.getTime() + 300_000;
      const hits = detections.filter(
        (d) => sensor.firmsSatelliteCodes.includes(d.satelliteCode) &&
               d.time.getTime() >= from && d.time.getTime() <= to
      ).length;
      if (hits === 0) silent++; else productive++;
      console.log(
        `  ${sensor.name.padEnd(11)} ${fmt(w[0].time)}-${fmt(w[w.length - 1].time)}  ` +
  ...
  const missBySensor = new Map<string, number>();
  const fractions: number[] = [];

  for (const d of detections) {
    const sensor = SENSORS.find((s) => s.firmsSatelliteCodes.includes(d.satelliteCode));
    if (!sensor) continue;
    const windows = windowsBySensor.get(sensor.name) ?? [];
    const w = windows.find(
      (win) => d.time.getTime() >= win[0].time.getTime() - 300_000 &&
               d.time.getTime() <= win[win.length - 1].time.getTime() + 300_000
    );
    if (!w) { noWindow++; continue; }

    checked++;
    const result = coverageForPoint(w, d.lat, d.lon, sensor.scanAngleDeg);
    if (result.covered) {
      covered++;
      fractions.push(result.fraction);
      if (result.nearNadir) nadir++; else edge++;
    } else {
  ...
  if (missBySensor.size) console.log('  misses by sensor:', Object.fromEntries(missBySensor));

  if (fractions.length) {
    console.log(`\n  near-nadir (<=${NADIR_FRACTION_LIMIT}) : ${nadir}  (${((100 * nadir) / fractions.length).toFixed(1)}%)  strong evidence`);
    console.log(`  scan edge  (>${NADIR_FRACTION_LIMIT})  : ${edge}  (${((100 * edge) / fractions.length).toFixed(1)}%)  weak evidence`);
    fractions.sort((a, b) => a - b);
    const bucket = [0, 0, 0, 0, 0];
    for (const f of fractions) bucket[Math.min(4, Math.floor(f * 5))]++;
