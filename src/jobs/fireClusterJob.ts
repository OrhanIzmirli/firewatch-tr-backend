...
class FireClusterJob {
   ...
   */
  private async buildClusters(client: any): Promise<ClusterRow[]> {
    const result = await client.query(
      ...
      [CLUSTER_EPS_METRES, CLUSTER_MIN_POINTS]
    );

    return result.rows.map((row: any) => ({
      detectionIds: row.detection_ids as number[],
      firstDetectedAt: row.first_detected_at as Date,
      lastDetectedAt: row.last_detected_at as Date,
      detectionCount: row.detection_count as number,
      overpassTimes: row.overpass_times as Date[],
   ...
   */
  private async findOpenIncident(client: any, cluster: ClusterRow): Promise<number | null> {
    const result = await client.query(
      `SELECT id
       FROM fire_incidents
       ...
       LIMIT 1`,
      [cluster.footprintWkt, INCIDENT_MATCH_METRES]
    );
  ...
  private async linkDetections(client: any, incidentId: number, ids: number[]): Promise<void> {
    await client.query(
      `UPDATE fire_detections SET incident_id = $1 WHERE id = ANY($2::bigint[])`,
      [incidentId, ids]
    );
  }
   ...
   */
  private async refreshSpread(client: any): Promise<number> {
    const result = await client.query(
      `WITH halves AS (
