...
router.post(
  '/nearest-city/batch',
  rateLimit('nearest-city-batch', 30, 60_000),
  async (req: Request, res: Response) => {
    try {
      ...
      const parsed = coords.map((c: any) => ({ lat: Number(c?.lat), lng: Number(c?.lng) }));
      ...
      const results: Array<Record<string, unknown> | null> = parsed.map((c) =>
        isWithinTurkey(c.lat, c.lng)
          ? null
      ...
      if (insideIdx.length > 0) {
        ...
        const query = await pool.query(
          `SELECT pts.idx, c.name, c.region, c.distance_km
