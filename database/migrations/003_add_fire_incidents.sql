-- Migration 003 — cluster detections into fire incidents
--
-- ADDITIVE. Creates two tables and one NOT VALID foreign key. Nothing
-- existing is dropped, retyped or rewritten.
--
-- ---------------------------------------------------------------------------
...
--   -- Expect one row.
--
--   SELECT to_regclass('public.fire_detections') AS must_exist,
--          to_regclass('public.fire_incidents')  AS must_be_null,
--          to_regclass('public.country_borders') AS must_be_null_too;
--   -- fire_detections must be a name (migration 002 applied);
--   -- the other two must be NULL.
--
--   SELECT count(*) AS detections_waiting FROM fire_detections;
--   -- Informational: these all get clustered on the first job run.
-- ---------------------------------------------------------------------------

BEGIN;

-- ===========================================================================
-- Country border, used as the country gate for ingest.
--
-- Natural Earth 1:50m admin-0, which is public domain (no attribution
-- required, though credit is appreciated): https://www.naturalearthdata.com
--
-- geom_gate is the border buffered outward by 5 km. The gate deliberately
-- errs wide: a 1:50m outline is generalised by up to a couple of kilometres,
-- and clipping a real Turkish detection off a coastline or a border valley is
-- a safety failure, whereas keeping a few Syrian pixels is cosmetic. The
-- exact outline is kept alongside it so a stricter test is available later.
-- ===========================================================================
...
CREATE TABLE IF NOT EXISTS fire_incidents (
  ...
  -- Every distinct acquisition time this incident has ever been seen at.
  -- Kept as a set rather than a counter because an incident is extended in
  -- many passes and the same overpass can arrive in more than one cluster —
  -- incrementing a counter would double-count it. Storing the times also
  -- keeps the total correct after the 90-day detection prune.
  overpass_times        TIMESTAMPTZ[] NOT NULL DEFAULT '{}',
  -- Cardinality of overpass_times, denormalised for cheap reads. Spread is
  -- only trusted with >= 2.
  overpass_count        INTEGER NOT NULL DEFAULT 0,

  centroid              geometry(Point, 4326) NOT NULL,
  -- Every pixel ever attributed to this incident. Stored on the incident and
  -- merged incrementally, never recomputed from fire_detections, so the
  -- 90-day detection prune cannot shrink a historical incident's footprint.
  footprint             geometry(MultiPoint, 4326) NOT NULL,

  max_frp_mw            NUMERIC(9,2),
  peak_confidence_tier  TEXT,
  city_id               INTEGER,
  region_key            TEXT,

  -- ---- AXIS 1: satellite thermal --------------------------------------
  satellite_state            TEXT NOT NULL,
  hours_since_last_detection NUMERIC(10,2) NOT NULL,

  -- ---- AXIS 2: satellite optical (EFFIS) ------------------------------
  -- Schema only for now; the EFFIS WFS integration lands in phase B. Adding
  -- the columns here avoids a second migration against a growing table.
  optical_burnt_area_ha NUMERIC(12,2),
  optical_fire_date     TIMESTAMPTZ,
  optical_last_update   TIMESTAMPTZ,
  optical_source        TEXT,
  optical_matched_at    TIMESTAMPTZ,

  -- ---- AXIS 3: official declaration -----------------------------------
  official_state        TEXT,
  official_source       TEXT,
  official_source_url   TEXT,
  official_confirmed_at TIMESTAMPTZ,
  official_note         TEXT,

  -- ---- movement (low confidence by construction) ----------------------
  spread_bearing_deg    NUMERIC(6,2),
  spread_speed_mh       NUMERIC(12,2),
  spread_confidence     TEXT NOT NULL DEFAULT 'insufficient',

  created_at            TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at            TIMESTAMPTZ NOT NULL DEFAULT NOW(),

  -- The safety constraint. 'extinguished' is NOT in this value set, so no
  -- satellite-derived code path can express it even by accident.
  CONSTRAINT fire_incidents_satellite_state_check
    CHECK (satellite_state IN ('detected_recently', 'no_recent_detection')),

  CONSTRAINT fire_incidents_official_state_check
    CHECK (official_state IS NULL
           OR official_state IN ('ongoing', 'contained', 'extinguished')),

  -- An official state is meaningless without provenance. This makes it
  -- structurally impossible to mark a fire contained or extinguished without
  -- naming the source, linking it and timestamping it — a satellite job has
  -- none of those to offer.
  CONSTRAINT fire_incidents_official_provenance_check
    CHECK (official_state IS NULL
           OR (official_source IS NOT NULL
               AND official_source_url IS NOT NULL
               AND official_confirmed_at IS NOT NULL)),

  CONSTRAINT fire_incidents_spread_confidence_check
    CHECK (spread_confidence IN ('insufficient', 'low', 'moderate')),

  -- No bearing or speed may be published while confidence is 'insufficient';
  -- an unknown direction must read as unknown, not as north.
  CONSTRAINT fire_incidents_spread_values_check
    CHECK (spread_confidence <> 'insufficient'
           OR (spread_bearing_deg IS NULL AND spread_speed_mh IS NULL))
);

CREATE INDEX IF NOT EXISTS idx_fire_incidents_centroid
  ON fire_incidents USING GIST (centroid);
CREATE INDEX IF NOT EXISTS idx_fire_incidents_footprint
  ON fire_incidents USING GIST (footprint);
CREATE INDEX IF NOT EXISTS idx_fire_incidents_last_detected
  ON fire_incidents (last_detected_at DESC);
CREATE INDEX IF NOT EXISTS idx_fire_incidents_region
  ON fire_incidents (region_key, last_detected_at DESC);
CREATE INDEX IF NOT EXISTS idx_fire_incidents_city
  ON fire_incidents (city_id, last_detected_at DESC);
-- Incident matching looks only at recently-active, unofficially-closed rows.
CREATE INDEX IF NOT EXISTS idx_fire_incidents_open
  ON fire_incidents (last_detected_at DESC) WHERE official_state IS NULL;

-- Migration 002 left incident_id without a foreign key because this table did
-- not exist yet. NOT VALID adds the constraint without scanning existing
-- rows, so it does not lock fire_detections while it grows.
DO $BLOCK$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fire_detections_incident_fk'
  ) THEN
    ALTER TABLE fire_detections
      ADD CONSTRAINT fire_detections_incident_fk
...
--   WHERE conrelid IN ('fire_incidents'::regclass, 'fire_detections'::regclass)
--   ORDER BY conname;
--
--   -- The border loaded and covers the country:
--   SELECT iso_a3, ST_NPoints(geom) AS vertices,
--          round((ST_Area(geom::geography) / 1e6)::numeric) AS km2
--   FROM country_borders;
--   -- Expect roughly 780,000 km2.
--
--   -- These two MUST both fail. If either succeeds, stop and re-check the
--   -- constraints above before letting the job run:
--   --   INSERT INTO fire_incidents (satellite_state, ...) VALUES ('extinguished', ...);
--   --   UPDATE fire_incidents SET official_state = 'extinguished';
--
-- ROLLBACK (only while both tables are unused):
--   ALTER TABLE fire_detections DROP CONSTRAINT IF EXISTS fire_detections_incident_fk;
