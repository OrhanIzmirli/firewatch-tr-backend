...
exports.NADIR_FRACTION_LIMIT = exports.SENSORS = void 0;
exports.halfSwathKm = halfSwathKm;
exports.distanceKm = distanceKm;
exports.crossTrackDistanceKm = crossTrackDistanceKm;
exports.parseTle = parseTle;
exports.groundTrack = groundTrack;
exports.coverageForPoint = coverageForPoint;
 ...
 */
exports.NADIR_FRACTION_LIMIT = 0.6;
 ...
 */
function halfSwathKm(altKm, scanAngleDeg) {
 ...
 */
function crossTrackDistanceKm(aLat, aLon, bLat, bLon, pLat, pLon) {
    ...
    if (dat < 0 || Number.isNaN(dat))
        return distanceKm(aLat, aLon, pLat, pLon);
    if (dat > segment)
        return distanceKm(bLat, bLon, pLat, pLon);
    return Math.abs(dxt) * EARTH_RADIUS_KM;
}
function parseTle(text, objectName) {
 ...
 */
function groundTrack(satrec, start, end, stepSeconds) {
    const points = [];
 ...
 */
function coverageForPoint(track, latDeg, lonDeg, scanAngleDeg) {
    if (track.length < 2)
        return NOT_COVERED;
    ...
    if (!Number.isFinite(best) || bestHalf === 0)
        return NOT_COVERED;
    ...
    return {
        ...
        nearNadir: fraction <= exports.NADIR_FRACTION_LIMIT,
        at: bestAt,
    };
}
