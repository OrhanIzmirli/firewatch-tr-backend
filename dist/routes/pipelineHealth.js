 ...
 */
router.get('/pipeline', (0, security_1.rateLimit)('pipeline_health', 60, 60000), async (_req, res) => {
    try {
        const result = await database_1.default.query(`SELECT j.job,
        ...
        const jobs = Object.values(EXPECTED).map((expected) => {
            ...
            const stats = (row?.last_success_stats ?? {});
            ...
            const producedOutput = Number(stats.incidentsCreated ?? 0) +
                Number(stats.incidentsExtended ?? 0);
            const anomaly = expected.job === 'cluster' && consideredInput > 0 && producedOutput === 0;
            return {
                job: expected.job,
                healthy,
                anomaly,
        ...
        if (error.code === '42P01') {
            ...
            res.status(200).json({
                status: 'unknown',
                message: 'job_runs table missing — apply ' +
